...
{
      <#
      $interface = $nic_hash."$68_subnet[0]"
      $x=$68_subnet[0]
      Write-Host "nic_hash.x" $nic_hash."$x"
      #>

    #  sleep time in seconds
    $SleepBetweenTeamChanges = 5  ## standard value = 5 sec
    Write-Host "Start creating teams..............."

    $ethernet_info = @(Get-NetIPAddress -AddressFamily IPv4) # Get all available Ethernet info
    #$cmd > Get-NetIpAddress.txt
    ...
    if ( $68_subnet.Length -gt 1) # create nic-team if more than one .68 subnet IP is present
    {
        $NIC1 = $68_subnet[0]
        $NIC2 = $68_subnet[1]
        #write-host $nic_hash."$NIC2" ":" $nic_hash."$NIC1"
        #write-host $NIC2 ":" $NIC1
        New-NetLbfoTeam -Name "68-nic" -TeamMembers $nic_hash."$NIC1", $nic_hash."$NIC2" -Confirm:$false | Out-Null
     ...
     if ( $61_subnet.Length -gt 1) # create nic-team if more than one .61 subnet IP is present
    {
        $NIC1 = $61_subnet[0]
        $NIC2 = $61_subnet[1]
     ...
     if ( $59_subnet.Length -gt 1) # create nic-team if more than one .59 subnet IP is present
    {
        $NIC1 = $59_subnet[0]
        $NIC2 = $59_subnet[1]
        #write-host $nic_hash."$NIC2" ":" $nic_hash."$NIC1"
        #write-host $NIC2 ":" $NIC1
        New-NetLbfoTeam -Name "59-nic" -TeamMembers $nic_hash."$NIC1", $nic_hash."$NIC2" -Confirm:$false | Out-Null
        sleep $SleepBetweenTeamChanges 
        if ($? -eq $false)   {
               $msg = "ERROR: Team creation for $tn failed on try $teamCreatedTryCount ."
               Write-Error -msgS E -msgText "Failed to create New-NetLbfoTeam"
